#!/usr/bin/env python3
"""
build_epub.py — Converte a tese LaTeX para EPUB via pandoc

Uso:
    python scripts/build_epub.py              # conversão completa
    python scripts/build_epub.py --outname meu_nome  # nome customizado
    python scripts/build_epub.py --verbose    # mostra saída completa do pandoc
    python scripts/build_epub.py --timestamp  # nome com timestamp
...
OUT_DIR = REPO_ROOT / "text" / "epub"
MAIN = "tese"

# Pandoc flags for LaTeX → EPUB
PANDOC_FLAGS: list[str] = [
    ...
    "--toc-depth=3",
    "--epub-chapter-level=1",  # each chapter starts a new spine item
]
...
def build(
    *,
    ...
    """Convert tese.tex → tese.epub using pandoc."""
    ...
    effective_outname = effective_outname.removesuffix(".epub")

    dst = OUT_DIR / f"{effective_outname}.epub"
    ...
    if not dst.exists():
        print("[ERRO] EPUB nao foi gerado.", file=sys.stderr)
        return 1

    size_kb = dst.stat().st_size // 1024
    print()
    print("[OK] EPUB gerado com sucesso:")
    print(f"     {dst}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Converte a tese LaTeX → EPUB via pandoc"
    )
    parser.add_argument(
        "--outname",
        default=None,
        help="Nome do EPUB em text/epub (sem extensao). Default: tese",
    )
    parser.add_argument(
