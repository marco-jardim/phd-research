...
A predominância de modelos baseados em árvores de decisão na literatura de \textit{record linkage} tabular levanta a questão sobre o desempenho de alternativas de aprendizado profundo quando aplicadas a dados de pares de similaridade.
Para avaliar essa hipótese de forma empírica, cinco classificadores foram treinados sob validação cruzada estratificada $5 \times 5$ (25 partições), seguida de avaliação em conjunto independente (\textit{holdout}, 30\% do total).
Os modelos comparados foram: \textit{Random Forest} (RF), XGBoost, duas arquiteturas de perceptron multicamada (MLP com camadas ocultas de 128--64 e 64--32 neurônios) e TabNet \cite{Arik2021tabnet}, rede neural com mecanismo de atenção projetada especificamente para dados tabulares.
...
Esse achado corrobora a evidência recente de que métodos de \textit{gradient boosting} e florestas aleatórias tendem a igualar ou superar redes neurais em tarefas tabulares de pequena e média escala, sobretudo quando a proporção de sinais positivos é baixa \cite{Grinsztajn2022trees, ShwartzZiv2022tabular}.
A convergência entre a fundamentação teórica e o resultado empírico reforça a escolha do XGBoost como classificador principal do \textit{pipeline} proposto.
