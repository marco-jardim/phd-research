...
O motor decisório substitui limiares fixos por minimização de \textit{expected loss}, com custos $C_{FN}$, $C_{FP}$ e $C_{LLM}$, além de taxas residuais $e_{FP}$ e $e_{FN}$ do módulo de revisão. Para um par com probabilidade calibrada $p$,
\[
...
O custo unitário por par é estritamente crescente ao longo da cascata, percorrendo sete ordens de grandeza entre a etapa~1 ($\approx$\,80\,ns, comparação numérica elementar) e a etapa~4 ($\approx$\,5\,s, inferência por modelo de linguagem com protocolo \textit{dual-agent} e possível arbitragem). Essa hierarquia é intencional e constitui o princípio arquitetural central do \textit{framework}: componentes de baixo custo e alta vazão eliminam a maior parcela do volume nos estágios iniciais, de modo que o recurso computacional mais oneroso seja reservado para o subconjunto residual em que a incerteza justifica o investimento.

O efeito combinado de custo unitário crescente e volume decrescente mantém o custo total das três primeiras etapas em magnitude comparável (5--50\,ms), com salto concentrado na etapa~4 ($\approx$\,200\,s), que ainda assim processa apenas 0,07\% do universo original. A Figura~\ref{fig:cap8-custo-computacional} ilustra essa dinâmica: o painel~(a) apresenta o custo unitário por par em escala logarítmica, evidenciando a progressão de sete ordens de grandeza; o painel~(b) apresenta o custo total por etapa, no qual se observa que a redução drástica de volume nas etapas iniciais compensa o aumento de custo unitário nas etapas posteriores.
...
O argumento econômico da cascata torna-se visível quando se considera o cenário contrafactual: a submissão direta dos 61.696 pares candidatos ao módulo de revisão assistida demandaria aproximadamente 85~horas de processamento contínuo (a 5\,s/par com protocolo \textit{dual-agent}) e custo de API na ordem de dezenas a centenas de dólares por lote, valores incompatíveis com a rotina de vigilância epidemiológica. Com a cascata, o mesmo resultado é alcançado em aproximadamente 26~minutos no modo vigilância (70\,ms para as etapas~1 a~3 e $\approx$\,26\,min para a revisão dos 1.410~pares selecionados na etapa~4), o que representa redução de cerca de três ordens de grandeza no tempo e de mais de 99\% no custo de inferência. A cascata opera, portanto, como mecanismo de contenção econômica que viabiliza o uso de modelos de linguagem sem que o custo de inferência se torne fator limitante.

Essa eficiência computacional possui implicação direta para a prática epidemiológica no contexto brasileiro. O Sistema Único de Saúde opera de forma descentralizada em mais de 5.500~municípios, dos quais a maioria dispõe de estações de trabalho convencionais sem acesso a servidores dedicados de processamento~\cite{Paim2011brazilian}. A arquitetura em cascata do GZ-CMD viabiliza a execução do \textit{pipeline} completo nesse cenário: as etapas~1 a~3, que resolvem 99,93\% dos pares, demandam apenas operações vetorizadas sobre memória local, realizáveis em qualquer computador contemporâneo em menos de um segundo; a etapa~4 requer acesso a API de modelo de linguagem, porém para volume residual de dezenas de pares, cujo custo de inferência é compatível com orçamentos de programas municipais de vigilância. Essa configuração pode contribuir para ampliar o acesso a técnicas de \textit{linkage} qualificado para além dos centros de pesquisa e capitais que tradicionalmente concentram capacidade técnica, favorecendo a descentralização da qualificação de dados de mortalidade e vigilância da tuberculose no âmbito do SUS.
