...
Foi adotada validação cruzada estratificada em cinco partições, com estratificação por COMPREC, mantendo coerência com o protocolo metodológico descrito no Capítulo~\ref{cap:metodo} (Seção~\ref{sec:analises-complementares}). Em cada rodada, quatro partições foram usadas para treinamento e calibração, e uma para teste. As métricas foram reportadas como média e desvio-padrão entre rodadas, com comparações por teste t pareado ($\alpha=0{,}05$).
