...
\hyphenation{apro-xi-ma-da-men-te}
...
Esta tese propôs e avaliou um arcabouço (\textit{framework}) configurável de pós-processamento, baseado em aprendizado de máquina (\textit{machine learning}), para a classificação de pares produzidos pelo \textit{linkage} probabilístico entre o Sistema de Informação sobre Mortalidade (SIM) e o Sistema de Informação de Agravos de Notificação para tuberculose (Sinan-TB) no município do Rio de Janeiro, no período de 2006 a 2016.
...
Do ponto de vista epidemiológico, a abordagem de máximo \textit{recall} (sensibilidade) recuperou 24 óbitos por tuberculose adicionais ($+$55,8\%) que teriam sido perdidos pelo limiar convencional, a um custo de aproximadamente 1,0 revisão manual por óbito detectado. A tese disponibiliza duas sequências operacionais (\textit{pipelines}) pré-configuradas, um orientado à vigilância (máximo \textit{recall}) e outro à confirmação (máxima precisão), articulados por uma fronteira de Pareto que permite ao gestor ou pesquisador calibrar o equilíbrio entre sensibilidade e especificidade conforme as necessidades do cenário de aplicação.

Adicionalmente, a tese propôs o \textit{Grey-Zone Cost-based Mixture Deferral} (GZ-CMD), arcabouço operacional auto-calibrável que estende o \textit{framework} ao integrar calibração por conjuntos âncora, política de decisão por perda esperada com custos assimétricos e revisão assistida por modelo de linguagem de grande porte. No modo de vigilância, o GZ-CMD alcançou $F_1$=0,954; no modo de confirmação, precisão de 0,957. Considerando a cascata completa do \textit{pipeline} , composta por limiares Fellegi--Sunter, regras de guarda, motor de perda esperada e revisão LLM dual-agent, o volume de revisão clerical humana reduziu-se de 21.620 para apenas 41~pares (redução de 99,8\%, cerca de três ordens de grandeza), processados em aproximadamente 63~minutos de inferência.
