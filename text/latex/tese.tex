...
A classificação dos pares candidatos resultantes desse processo, contudo, permanece dependente de limiares fixos aplicados sobre escores agregados de similaridade, o que acarreta perda expressiva de registros situados na zona de indecisão (zona cinzenta).
...
Esta tese propôs e avaliou um \textit{framework} configurável de pós-processamento, baseado em aprendizado de máquina (\textit{machine learning}), para a classificação de pares produzidos pelo \textit{linkage} probabilístico entre o Sistema de Informação sobre Mortalidade (SIM) e o Sistema de Informação de Agravos de Notificação para tuberculose (Sinan-TB) no município do Rio de Janeiro, no período de 2006 a 2016.
A base experimental compreendeu 61.696 pares candidatos gerados pelo OpenRecLink e pontuados por um comparador de registros que produz 29 subescores de similaridade e um escore agregado, tendo como padrão-ouro 247 pares verdadeiros (razão $\approx$1:249).
No estudo de ablação inicial, seis algoritmos foram comparados (Regressão Logística, Floresta Aleatória, XGBoost, LightGBM, SVM e Perceptron Multicamada), combinados com nove estratégias de balanceamento de classes (SMOTE, Borderline-SMOTE, ADASYN, SMOTE-Tomek, ponderação de classes e variantes), totalizando 70 configurações. Na comparação final entre famílias de classificadores, cinco modelos foram retidos para análise aprofundada.

A Floresta Aleatória (\textit{Random Forest}) com SMOTE e limiar de classificação de 0,5 alcançou $F_1$=0,931, ao passo que o limiar convencional $\geq$8 obteve $F_1$=0,610. A comparação entre famílias de classificadores identificou o XGBoost como alternativa com desempenho equivalente ou superior ($F_1 = 0{,}943$ no \textit{hold-out}; $0{,}924 \pm 0{,}030$ na validação cruzada), sendo adotado como classificador padrão do \textit{framework}.
...
A interpretação por meio de valores SHAP identificou o escore agregado, a quantidade de nomes iguais e o indicador de nome perfeito como os atributos de maior poder discriminatório; o nome da mãe predominou nas decisões relativas à zona cinzenta.
A análise de equidade, inspirada no critério de \textit{equalized odds}, indicou homogeneidade da taxa de verdadeiros positivos entre subgrupos demográficos e de similaridade, com disparidades de valor preditivo positivo atribuíveis a diferenças de prevalência, e não a viés discriminatório do modelo.
Do ponto de vista epidemiológico, a abordagem de máxima sensibilidade recuperou 24 óbitos por tuberculose adicionais ($+$55,8\%) que teriam sido perdidos pelo limiar convencional, a um custo de aproximadamente 1,0 revisão manual por óbito detectado. A tese disponibiliza duas sequências operacionais (\textit{pipelines}) pré-configuradas, um orientado à vigilância (máxima sensibilidade) e outro à confirmação (máxima precisão), articulados por uma fronteira de Pareto que permite ao gestor ou pesquisador calibrar o equilíbrio entre sensibilidade e especificidade conforme as necessidades do cenário de aplicação.

Adicionalmente, a tese propôs o \textit{Grey-Zone Cost-based Mixture Deferral} (GZ-CMD), \textit{framework} operacional auto-calibrável que estende o \textit{framework} ao integrar calibração por conjuntos âncora, política de decisão por \textit{expected loss} com custos assimétricos e revisão assistida por modelo de linguagem de grande porte. No modo de vigilância, o GZ-CMD alcançou $F_1$=0,954; no modo de confirmação, precisão de 0,957. Considerando a cascata completa do \textit{pipeline} , composta por limiares Fellegi--Sunter, regras de guarda, \textit{expected loss engine} e revisão LLM dual-agent, o volume de revisão clerical humana reduziu-se de 21.620 para apenas 41~pares (redução de 99,8\%, cerca de três ordens de grandeza), processados em aproximadamente 63~minutos de inferência.
...
The experimental dataset comprised 61,696 candidate pairs generated by OpenRecLink and scored by a record comparator that produces 29 similarity sub-scores and an aggregate score, with a gold standard of 247 true pairs (ratio $\approx$1:249).
...
Random Forest with SMOTE and a classification threshold of 0.5 achieved $F_1$=0.931, whereas the conventional threshold $\geq$8 yielded $F_1$=0.610. A comparison across classifier families identified XGBoost as an alternative with equivalent or superior performance ($F_1 = 0.943$ on the hold-out set; $0.924 \pm 0.030$ in cross-validation), leading to its adoption as the default classifier of the framework.
...
A fairness analysis inspired by the equalized-odds criterion showed that the true positive rate remained homogeneous across demographic and similarity subgroups, with positive predictive value disparities attributable to prevalence differences rather than model bias.
From an epidemiological standpoint, the maximum-recall approach recovered 24 additional tuberculosis deaths ($+$55.8\%) that would have been missed by the conventional threshold, at a cost of approximately 1.0 manual reviews per detected death. The thesis delivers two pre-configured pipelines, one geared towards surveillance (maximum recall) and another towards confirmation (maximum precision), articulated through a Pareto frontier that allows the health manager or researcher to calibrate the trade-off between sensitivity and specificity according to the requirements of the application scenario.

Additionally, the thesis proposed the Grey-Zone Cost-based Mixture Deferral (GZ-CMD), a self-calibrating operational framework that extends the base framework by integrating anchor-based calibration, an asymmetric expected loss decision policy, and large language model-assisted review. In surveillance mode, GZ-CMD achieved $F_1$=0.954; in confirmation mode, precision of 0.957. Considering the full pipeline cascade , comprising Fellegi--Sunter thresholds, deterministic guardrails, expected loss engine, and dual-agent LLM review, the human clerical review volume was reduced from 21,620 to only 41~pairs (99.8\% reduction, approximately three orders of magnitude), processed in approximately 63~minutes of inference.
