...
\item[Calibra\c{c}\~{a}o de Platt] T\'{e}cnica de p\'{o}s-processamento que ajusta os escores brutos de um classificador por meio de regress\~{a}o log\'{i}stica (fun\c{c}\~{a}o sigmoide), convertendo-os em probabilidades calibradas (\textit{Platt scaling}).
...
\item[GZ-CMD] \textit{Grey-Zone Cost-based Mixture Deferral}. Arcabou\c{c}o operacional proposto nesta tese para governan\c{c}a da incerteza no \textit{linkage} probabil\'{i}stico, integrando calibra\c{c}\~{a}o por conjuntos \^{a}ncora, regras de guarda, motor de perda esperada com custos assim\'{e}tricos e revis\~{a}o assistida por modelo de linguagem em pol\'{i}tica \'{u}nica de decis\~{a}o audit\'{a}vel.
...
\item[Motor de perda esperada] Componente decis\'{o}rio do GZ-CMD que substitui limiares fixos por minimiza\c{c}\~{a}o de perda esperada com custos assim\'{e}tricos ($C_{FP}$, $C_{FN}$, $C_{LLM}$), determinando para cada par se o valor esperado da revis\~{a}o por modelo de linguagem supera o custo da decis\~{a}o autom\'{a}tica.
...
\item[Regras de guarda] Conjunto de filtros determin\'{i}sticos aplicados antes do motor de perda esperada no GZ-CMD, incluindo filtro temporal (diferen\c{c}a entre \'{o}bito e diagn\'{o}stico superior a 180 dias), limiar inferior de nota global e filtro de homonimia.
