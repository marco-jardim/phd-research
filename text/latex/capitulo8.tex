...
O \textit{framework} (\textit{framework}) GZ-CMD, apresentado no Capítulo~\ref{cap:gzcmd}, estendeu a abordagem de pós-processamento ao incorporar calibração por âncoras, política de decisão em três vias (aceitar, rejeitar e revisar) fundamentada no valor esperado de revisão e revisão assistida por modelos de linguagem de grande porte (\textit{large language models}) para os pares residuais da zona cinzenta. A configuração orientada à vigilância atingiu F$_1$-Score de 0,954, enquanto a orientada à confirmação alcançou precisão de 0,957 (F$_1$=0,942), refletindo a ênfase de cada modo operacional na métrica que lhe é prioritária. Ao percorrer as quatro etapas da cascata, isto é, limiares Fellegi--Sunter, regras de guarda determinísticas, \textit{expected loss engine} e revisão \textit{dual-agent} por modelo de linguagem, o volume de revisão clerical reduziu-se de 21.620 para 41~pares (99,8\%, cerca de três ordens de grandeza). O tempo total de processamento do \textit{pipeline} completo, incluindo as chamadas à API do modelo de linguagem para os 1.410 pares encaminhados à etapa 4 com 15 requisições concorrentes, foi de aproximadamente 18~minutos no modo vigilância.
