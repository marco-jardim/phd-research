...
A regra de decisão ótima $\delta^*(p)$ é aquela que minimiza a \textit{expected loss}:
...
Para operacionalizar a triagem sob restrição orçamentária, define-se o Valor Esperado da Revisão (\textit{Expected Value of Review}, EVR). O EVR quantifica a redução de \textit{expected loss} obtida ao se optar pela revisão em vez da melhor decisão automática disponível:
...
\begin{proposition}
No caso especial em que (a) os custos são simétricos ($C_{FP} = C_{FN} = C$), (b) o revisor é perfeito ($e_{FP} = e_{FN} = 0$), e (c) o custo de revisão tende a zero ($C_{LLM} \to 0$), a decisão por \textit{expected loss} mínima $\delta^*(p)$ converge para a decisão por máxima verossimilhança (limiar em $p = 0{,}5$), e a zona de revisão colapsa ao conjunto vazio.
\end{proposition}
